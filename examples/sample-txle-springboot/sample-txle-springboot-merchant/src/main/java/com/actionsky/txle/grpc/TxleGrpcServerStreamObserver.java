...
class TxleGrpcServerStreamObserver implements StreamObserver<TxleGrpcServerStream> {
    ...
    private final IntegrateTxleService integrateTxleService;

    TxleGrpcServerStreamObserver(IntegrateTxleService integrateTxleService) {
        this.integrateTxleService = integrateTxleService;
    }
    ...
    public void onNext(TxleGrpcServerStream serverStream) {
        ...
        try {
            ...
            serverStream.getExecuteSqlList().forEach(executeSql -> {
                ...
                try {
                    integrateTxleService.getCustomRepository().executeSubTxSqls(executeSql.getSubTxSqlList());
                    isExecutedOK = true;
                } catch (Exception e) {
    ...
    public void onError(Throwable t) {
        try {
            integrateTxleService.onReconnect();
        } catch (Throwable e) {
            LOG.error("Failed to reconnect to txle server - {}.", integrateTxleService.getTxleGrpcServerAddress(), e);
        }
    }
