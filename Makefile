...
upload_jar_to_nexus:
	$(DOCKER) run -v $(pwd)/settings.xml:/usr/share/maven/conf/settings.xml --rm $(DOCKER_IMAGE) /bin/bash -c "mvn deploy -DskipTests -Pdev"

upload_jar_to_internet:
	$(DOCKER) run -v $(pwd)/settings-release.xml:/usr/share/maven/conf/settings.xml --rm $(DOCKER_IMAGE) /bin/bash -c "mvn deploy -DskipTests -Prelease"
