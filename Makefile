...
upload_jar_to_nexus:
	$(DOCKER) run -v $(shell pwd)/:/opt/code --rm -w /opt/code $(DOCKER_IMAGE) /bin/bash -c "cp settings.xml /usr/share/maven/conf/; mvn clean deploy -DskipTests -Pdev"

upload_jar_to_internet:
	$(DOCKER) run -v $(shell pwd)/:/opt/code --rm -w /opt/code $(DOCKER_IMAGE) /bin/bash -c "cp settings.xml /usr/share/maven/conf/; mvn clean deploy -DskipTests -Prelease"
