...
docker_mvn_txle:
	$(DOCKER) run -v $(shell pwd)/:/opt/code --rm -w /opt/code $(DOCKER_IMAGE) /bin/bash -c "mvn clean deploy -DskipTests -Pdev -gs settings.xml"

docker_mvn_txle_release:
	$(DOCKER) run -v $(shell pwd)/:/opt/code --rm -w /opt/code $(DOCKER_IMAGE) /bin/bash -c "mvn clean deploy -DskipTests -Prelease -gs settings-release.xml"
